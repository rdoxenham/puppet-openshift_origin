...
class openshift_origin::broker {
  ...
  if !defined(File['mcollective client config']) {
    ...
    file { 'mcollective client log':
      ensure  => present,
      path    => '/var/log/mcollective-client.log',
      owner   => 'apache',
      group   => 'root',
      mode    => '0644',
      require => Package['mcollective'],
  ...
  if $::openshift_origin::broker_auth_pub_key == 'create_new' {
    ...
    file { 'broker auth public key':
      ...
      content => $::openshift_origin::broker_auth_pub_key,
      owner   => 'root',
      group   => 'root',
      mode    => '0644',
    ...
    file { 'broker auth private key':
      ...
      content => $::openshift_origin::broker_auth_priv_key,
      owner   => 'root',
      group   => 'root',
      mode    => '0644',
      require => Package['rubygem-openshift-origin-controller'],
  ...
  if $::openshift_origin::broker_rsync_key == 'create_new' {
    exec { 'rsync ssh key':
    ...
    file { 'broker rsync private key':
      ensure  => present,
      path    => '/etc/openshift/rsync_id_rsa',
      content => $::openshift_origin::broker_rsync_key,
      owner   => 'root',
      group   => 'root',
      mode    => '0644',
      require => Package['rubygem-openshift-origin-controller'],
  ...
  file { 'broker servername config':
    ...
    path    => '/etc/httpd/conf.d/000002_openshift_origin_broker_servername.conf',
  ...
  exec { 'fixfiles rubygem-passenger':
    command     => '/sbin/fixfiles -R ruby193-rubygem-passenger restore && \
      /sbin/fixfiles -R ruby193-mod_passenger restore',
    subscribe   => Package['rubygem-passenger'],
    refreshonly => true,
  }

  exec { 'restorecon /var/run & /opt':
    command	=> '/sbin/restorecon -rv /var/run && /sbin/restorecon -rv /opt',
  }
