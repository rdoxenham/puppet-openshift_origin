...
# [*mongodb_fqdn*]
#   FQDN of node running the MongoDB server, including port (If running on a different node)
# [*mongodb_replicas*]
#   Whether or not to use replicas. Note that if this is true, you must update mongodb_fqdn accordingly
# [*mq_fqdn*]
...
class openshift_origin (
  ...
  $mongo_db_name              = 'openshift_broker',
  ...
  if $install_client_tools == true {
    ...
    if $::operatingsystem == 'Redhat' {
      ...
      ensure_resource('package', 'rubygem-net-ssh', {
        ensure => present,
      }
      )
      ensure_resource('package', 'rubygem-archive-tar-minitar', {
        ensure => present,
      }
      )
